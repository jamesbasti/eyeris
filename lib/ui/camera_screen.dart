    ...
    try {
      ...
      for (int y = 0; y < targetSize && y < height; y += 2) {
        for (int x = 0; x < targetSize && x < width; x += 2) {
          ...
          final index = sourceY * stride + sourceX * 4; // BGRA format
  ...
  List<String> _parseOutput(
    ...
    try {
      ...
      for (int i = 0; i < math.min(numDet, 10); i++) {
        ...
        if (score > 0.1) { // Very low initial threshold to catch more potential objects
      ...
      final foregroundDetections = sortedDetections.where((detection) {
        ...
        final isLargeEnough = area > 0.005; // Lower minimum size threshold
        final isHighConfidence = score > 0.2; // Much lower confidence threshold
